...
$sum_temp = 0;
$temps = [];
$count_hourly = count($hourly);
foreach ($hourly as $iValue) {
    $temp = $iValue->{'temp'}->{'metric'};
    if ($temp < $min_temp) {
        $min_temp = $temp;
    }
    if ($temp > $max_temp) {
...
$avg_temp = $sum_temp / count($hourly);
...
$temps_count = min(32, count($temps));
foreach ($temps as $i => $iValue) {
